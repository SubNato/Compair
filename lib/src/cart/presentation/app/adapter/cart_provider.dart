...
*/
    ...
    // Keep the current cart state to avoid full page reload
    final currentState = state;
    final currentCart = currentState is CartFetched ? currentState.cart : <CartProduct>[];

    //Loads when a single product is updated
    state = const ChangingCartProductQuantity();
    ...
    await result.fold(
          (failure) async => state = CartError(failure.errorMessage),
          (_) async {
        // Update local cart state optimistically without fetching full cart
        final updatedCart = currentCart.map((product) {
          if (product.id == cartProductId) {
            // Create updated product with new quantity
            return (product as CartProductModel).copyWith(quantity: newQuantity);
          }
          return product;
        }).toList();

        // Set state to CartFetched with updated cart to trigger checkout button update
        //state = CartFetched(updatedCart);

        state = CartQuantityUpdate(updatedCart);

        // Emit lightweight state to signal quantity was changed
        // This recalculates the total without triggering a full page reload
        ref.read(cartAdapterProvider(GlobalKeys.cartCountFamilyKey).notifier)
            ._getCartCount(userId);
      },
    );
    ...
    await result.fold(
      (failure) async => state = CartError(failure.errorMessage),
      (_) async {
        //Fetching the Cart details again so that the $amount is matching
        // with the quantity change. // Automatic reloads
        //=> state = const ChangedCartProductQuantity(),
        // ^ does not update the $amount till the page reloads manually

        final cartRes = await _getCart(userId);
        cartRes.fold(
          (failure) => state = CartError(failure.errorMessage),
          (cart) => state = CartFetched(cart),
        );
